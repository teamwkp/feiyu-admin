<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="电池类型" prop="batteryTypeId">
        <el-input
          v-model="queryParams.batteryTypeId"
          placeholder="请输入电池类型"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['commission:barCommission:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['commission:barCommission:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['commission:barCommission:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['commission:barCommission:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="barCommissionList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="状态" align="center" prop="id" />
      <el-table-column label="门店编号" align="center" prop="storenUmber" />
      <el-table-column label="标题" align="center" prop="title" />
      <el-table-column label="电池类型" align="center" prop="batteryTypeId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="电池类型" prop="batteryTypeId">
          <el-input v-model="form.batteryTypeId" placeholder="请输入电池类型" />
        </el-form-item>
        <el-form-item label="分佣类型" prop="ratio">
...
<script>
import { listBarCommission, getBarCommission, delBarCommission, addBarCommission, updateBarCommission, exportBarCommission } from "@/api/commission/barCommission";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        batteryTypeId: [
          { required: true, message: "电池类型不能为空", trigger: "blur" }
        ],
        ratio: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBarCommission(this.queryParams).then(response => {
        this.barCommissionList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getBarCommission(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateBarCommission(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBarCommission(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除电池佣金策略编号为"' + ids + '"的数据项？').then(function() {
        return delBarCommission(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有电池佣金策略数据项？').then(() => {
        this.exportLoading = true;
        return exportBarCommission(queryParams);
      }).then(response => {
