    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="协议类型" prop="protocolType">
        <el-select v-model="queryParams.protocolType" placeholder="请选择协议类型" clearable size="small">
          <el-option
            v-for="dict in dict.type.protocol_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['operational:protocol:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['operational:protocol:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['operational:protocol:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['operational:protocol:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="protocolList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="协议类型" align="center" prop="protocolType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.protocol_type" :value="scope.row.protocolType"/>
        </template>
      </el-table-column>
...
<script>
import { listProtocol, getProtocol, delProtocol, addProtocol, updateProtocol, exportProtocol } from "@/api/operational/protocol";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除协议内容编号为"' + ids + '"的数据项？').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有协议内容数据项？').then(() => {
